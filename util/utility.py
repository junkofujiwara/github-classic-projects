...
def init():
    ...
    try:
        ...
        github_token = None
        script = sys.argv[0]
        usage_text = (f"Usage: {script} [list]"
                "-o <organization_name> -r <repository> -t <github_personal_token>")
        operation = sys.argv[1]
        opts, _args = getopt.getopt(
            sys.argv[2:], "o:r:t:h", ["org=", "repo=", "token=", "help"]
        )
        for opt, arg in opts:
            if opt in ("-o", "--org"):
                github_org = arg
        ...
        if github_token is None:
            logging.info(usage_text)
            sys.exit()
        return github_org, github_repo, github_token, operation
